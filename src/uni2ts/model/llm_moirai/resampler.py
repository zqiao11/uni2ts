...
class LinearProjector(nn.Module):
    def __init__(self, in_features, out_features, inner_features=512, dropout=0, bias=True):
        super().__init__()
        self.model = nn.Sequential(nn.LayerNorm(in_features),
...
class PerceiverAttention(nn.Module):
    ...
    def forward(self, x, queries):
        ...
        q, k, v = rearrange(q, 'b t (h d) -> b h t d', h=h), rearrange(k, 'b t (h d) -> b h t d', h=h), rearrange(v, 'b t (h d) -> b h t d', h=h)
...
class PerceiverResampler(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pos_emb = nn.Parameter(torch.randn(max_seq_len, dim))  # Positional embeddings
