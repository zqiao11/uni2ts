...
from .proceed_adpter.down_up import add_down_up_
from .proceed_adpter.generator import AdaptGenerator


def add_adapters_(parent_module: nn.Module, freeze, merge_weights=True):
    for name, module in parent_module.named_children():
        if isinstance(module, (nn.Linear, )):
            add_down_up_(parent_module, name, freeze_weight=freeze, merge_weights=merge_weights,)
        else:
            add_adapters_(module, freeze, merge_weights)
    return parent_module


class ProceedMoiraiOnline(L.LightningModule):
    ...
    def __init__(
        ...
        # 存前1个完整样本?  我们这里就是target
        self.register_buffer(
            'recent_batch',
            torch.zeros(1, 7, context_length + prediction_length),  # num of variates
            persistent=False
        )

        # concept encoders
        self.mlp1 = nn.Sequential(nn.Linear(context_length, concept_dim), nn.GELU(),
                                  nn.Linear(concept_dim, concept_dim))

        self.mlp2 = nn.Sequential(nn.Linear(context_length + prediction_length, concept_dim), nn.GELU(),
                                  nn.Linear(concept_dim, concept_dim))

        self.concept_dim = concept_dim

    def post_init(self):
        # #  往backbone里加adapter, 把原Linear变成新定义的带adapter的Linear, freeze是指是否freeze原本module的weight
        # self.module = add_adapters_(self.module, freeze=False, merge_weights=True)
        #
        # self.generator = AdaptGenerator(
        #     self.module,                     # 加过adapter替换后的
        #     self.concept_dim,
        #     activation=nn.Sigmoid,
        #     adaptive_dim=False,
        #     need_bias=False,
        #     shared=True,
        #     mid_dim=self.concept_dim//4
        #     )
        pass
    ...
    def generate_adaptation(self, x):
        """
        根据context series x (bs, length)生成adapter参数
        """
        # 这两concept应该是(concept_dim)
        concept = self.mlp1(x).mean(0)
        recent_concept = self.mlp2(self.recent_batch).mean(-2).mean(list(range(0, self.recent_batch.dim() - 2)))
        drift = concept - recent_concept
        res = self.generator(drift, need_clip=True)
        return res

    def forward_with_proactive_adaption(
            self,
        ...
        # context = self._get_context_series(target)  # (7, 512)
        # adaptations = self.generate_adaptation(context)
        #
        # for out_dim, adaptation in adaptations.items():
        #     for i in range(len(adaptation)):
        #         name = self.generator.dim_name_dict[out_dim][i]
        #         self.module.get_submodule(name).assign_adaptation(adaptation[i])
    ...
    def training_step(
        ...
        # Proactive adapt the model before eval
        self.online_val_step(batch, batch_idx)
        self.recent_batch = self._get_whole_series(batch['target']).unsqueeze(0)   # ToDo: 更新，存(7, context+pred)

        # Online Finetune. Use model parameters wo adaptation.
        distr = self(
    ...
    def online_val_step(self, batch, batch_idx):
        ...
        # ToDo: 在这里让self.()变成用adapter
        distr = self.forward_with_proactive_adaption(
            **{field: batch[field] for field in list(self.seq_fields) + ["sample_id"]}
        )
    ...
    def _get_context_series(self, target):
    ...
    def _get_whole_series(self, target):
        context_series_with_pad = target[:, :self.num_ctx_patch, :self.patch_size].reshape(target.size(0), -1)
        context_series = context_series_with_pad[:, -self.context_length:]
        ...
        return torch.concatenate((context_series, pred_series), dim=1)
