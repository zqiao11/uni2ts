...
def seasonal_naive_predict_torch(
    past_target: torch.Tensor, prediction_length: int
) -> torch.Tensor:
    """
    Apply seasonal naive prediction using the past target time series (Torch version).

    Args:
        past_target (torch.Tensor): A tensor of shape (batch, past_time, tgt) containing the historical data.
        prediction_length (int): The length of the prediction horizon.

    Returns:
        torch.Tensor: Forecasted values of shape (batch, prediction_length, tgt) using seasonal naive method.
    """
    batch_size, past_time, tgt = (
        past_target.shape
    )  # Extract batch shape and target dimensions

    # Initialize future_target tensor
    future_target = torch.zeros(
        (batch_size, prediction_length, tgt),
    ...
    for b in range(batch_size):
        for i in range(tgt):
            # Convert the past time series into numpy array for FFT processing
            past_series_np = past_target[b, :, i].cpu().numpy()

            # Compute FFT on the past series
            fft_vals = np.fft.fft(past_series_np)
            freqs = np.fft.fftfreq(past_time)

            # Discard freq=0 (the mean component) and find the dominant frequency
            fft_vals = fft_vals[1:]
            freqs = freqs[1:]
            ...
            # ToDo: For now, we only consider the case that context is longer than predicton
            # Generate the future_target using seasonal naive prediction
            if period == past_time:
                # No clear seasonality, copy the last `prediction_length` points
                future_target[b, :, i] = past_target[b, -prediction_length:, i]
            else:
                # Apply seasonal naive prediction logic for `prediction_length` time steps
                for t in range(prediction_length):
                    future_target[b, t, i] = past_target[
                        b, (past_time - period + (t % period)) % past_time, i
                    ]
...
class MoiraiForecast(L.LightningModule):
    ...
    def _get_distr(
        ...
        # convert format. QZ: Target here is the seasonal naive target
        (
            target,
            observed_mask,
            sample_id,
