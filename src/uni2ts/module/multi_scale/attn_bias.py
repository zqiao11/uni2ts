...
class BinaryAttentionBias(AttentionBias):
    ...
    def post_init(self, num_scales: int):
        ...
        self.scale_emb = nn.ModuleList(
            [nn.Embedding(num_embeddings=self.num_heads, embedding_dim=self.embedding_dim) for _ in range(num_scales)]
        )
        # 初始化权重为零
        for embedding in self.scale_emb:
            nn.init.normal_(embedding.weight, mean=0, std=0.001)
    ...
    def forward(
        ...
        # q_emb = torch.empty((q_len, self.num_heads, self.embedding_dim), device=query_id.device)
        # kv_emb = torch.empty((kv_len, self.num_heads, self.embedding_dim), device=kv_id.device)
        ...
        # )
        #
        # # weight1 = self.scale_emb[0].weight
        # bias = bias + new_bias
