...
# 添加虚线方框（浅灰色，虚线，粗线）
i = 0
for start_idx in range(0, len(target[:ctx_len]), step_size):
    ...
    if end_idx <= len(target):
        ...
        if i == 1:
            plt.axvspan(start_idx + bias, end_idx - gap_size, color='gray', alpha=0.5)  # alpha 控制透明度，数值越小越透明
    i += 1

plt.xlim(0, len(target[:ctx_len]))
plt.yticks([])
...
# 添加虚线方框（浅灰色，虚线，粗线）
i = 0
for start_idx in range(0, len(context_ds2), int(step_size/2)):
    ...
    if end_idx <= len(context_ds2):
        ...
        if i == 1:
            plt.axvspan(start_idx + bias, end_idx - gap_size/2, color='gray', alpha=0.5)  # alpha 控制透明度，数值越小越透明
    i += 1
...
# 添加虚线方框（浅灰色，虚线，粗线）
i = 0
for start_idx in range(0, len(context_ds4), int(step_size / 4)):
    end_idx = start_idx + window_size / 4
    bias = 0.25 if start_idx == 0 else 0
    if end_idx <= len(context_ds4):
        ...
        if i == 1:
            plt.axvspan(start_idx + bias, end_idx - gap_size/4, color='gray', alpha=0.5)  # alpha 控制透明度，数值越小越透明
    i += 1
